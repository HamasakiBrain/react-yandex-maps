import type { HorizontalPosition, VerticalPosition, YandexMapMarkerCustomProps } from '../types/marker';
...
export function getMarkerContainerProps({
    ...
    const root: Record<string, any> = {
        className: ['__ymap-marker'].join(' '),
        style: {},
    };
    const children: Record<string, any> = {
        className: ['__ymap-marker_wrapper'].join(' '),
        style: {},
    };
    ...
    if (position && !isDefaultPosition) {
        if (position.startsWith('translate')) {
            (children.style as any).transform = position;
        }
        else {
            let translateX = 0;
            let translateY = 0;
            ...
            (children.style as any).transform = `translate(${ translateX }%, ${ translateY }%)`;
        }
    }

    if (zeroSizes === true || (zeroSizes !== false && position && !isDefaultPosition)) {
        (root.style as any).width = '0';
        (root.style as any).height = '0';

        if ((children.style as any).transform) (children.style as any).width = 'fit-content';
    }
    ...
    for (const [key, value] of Object.entries(attrs)) {
        ...
        if ((value as any).className) {
            obj.className = [obj.className, (value as any).className].filter(Boolean).join(' ');
            delete (value as any).className;
        }

        if ((value as any)?.style) {
            if (typeof (value as any).style !== 'object' || Array.isArray((value as any).style)) {
...
export function excludeYandexMarkerProps<T extends Record<string, any>>(props: T): Omit<T, keyof YandexMapMarkerCustomProps> {
    ...
    for (const excluded in toExclude) {
        if (excluded in props) delete (props as any)[excluded];
    }
