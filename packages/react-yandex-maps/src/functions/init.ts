...
import type { DefProductSettings, YandexMapPluginSettings } from '../utils/init';
...
export function initYmaps() {
    ...
    if (typeof window === 'undefined') return Promise.reject(new YandexMapException('You must call initYmaps on Client Side only'));
    if (loadPromise) return loadPromise;

    yandexMapLoadStatus.value = 'loading';

    loadPromise = new Promise<void>((res, rej) => {
        const settings = yandexMapSettings.value;
        if (!settings.apikey) {
            throwException({
                text: 'apikey was not set for Yandex Maps initialization. Ensure you have called createYmapsOptions with apikey.',
            });
        }
...
export function createYmapsOptions(options: YandexMapPluginSettings): YandexMapPluginSettings {
    // Only init once
