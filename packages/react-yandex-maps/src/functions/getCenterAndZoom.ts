...
import { getException, throwException } from '../utils/system';
...
export async function getLocationFromBounds({
    ...
    if (!ctxMap) {
        throwException({
            text: 'CtxMap was not found in getLocationFromBounds',
            isInternal: true,
        });
    }
    ...
    const ctxItem = await new Promise<any>((resolve, reject) => {
        ...
        reject(getException({
            text: 'Map item was not found in getLocationFromBounds',
            isInternal: true,
        }));
    });
    ...
    if (!ctxItemMapKey) {
        throwException({
            text: 'CtxMapKey was not found in getLocationFromBounds',
            isInternal: true,
        });
    }
